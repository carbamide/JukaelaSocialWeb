...
def repost
  respond_to do |format|
    ...
    format.json {
      ...
      @micropost = current_user.microposts.build(:name => mp.name, 
      :username => mp.username, 
      :email => mp.email, 
      :content => mp.content,
      :user_id => mp.user_id, 
      :repost_user_id => current_user.id, 
      :repost_name => current_user.name, 
      :repost_username => current_user.username, 
...
def images_from_feed
  ...
  Micropost.all.each do |m|
    if m.image_url
      ...
      return_hash["title"] = m.content.split[0..1].join(" ") + "!"
